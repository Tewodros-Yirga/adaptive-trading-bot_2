...
// ── Input ─────────────────────────────────────────────────────────────────
export const Input = ({
  label, value, onChange, type = 'text', min, max, step, disabled, placeholder,
}: {
  label: string; value: any; onChange: (v: any) => void
  type?: string; min?: number; max?: number; step?: number; disabled?: boolean; placeholder?: string
}) => (
  ...
  return (
    <div className="fixed top-4 right-4 z-50 flex flex-col gap-2">
      {toasts.map((t) => (
        <div
          key={t.id}
          className="slide-in bg-panel border border-border rounded-lg px-4 py-3 flex items-center gap-3 shadow-xl min-w-64"
        >
          {icons[t.type]}
          <span className="text-sm flex-1">{t.message}</span>
          <button onClick={() => removeToast(t.id)} className="text-muted hover:text-white">
            <X size={14} />
          </button>
        </div>
      ))}
  ...
  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
    <div className="bg-panel border border-border rounded-xl p-6 max-w-md w-full mx-4 fade-in">
      ...
      <div className="flex gap-3 justify-end">
        ...
        <Btn variant={variant === 'danger' ? 'danger' : 'default'} onClick={onConfirm}>Confirm</Btn>
      </div>
    </div>
  </div>
)
...
export const StatusDot = ({ live }: { live: boolean }) => (
  <span className={clsx(
    ...
    live ? 'bg-success pulse-dot' : 'bg-muted',
  )} />
)
