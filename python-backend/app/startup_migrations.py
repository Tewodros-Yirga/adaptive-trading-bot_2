...
These run at application boot using raw SQL so they work
whether or not Alembic has been run separately.
Each statement is idempotent (uses IF NOT EXISTS / checks information_schema).
"""
...
_ADD_COLUMN_STMTS = [
    ...
    BEGIN
        IF NOT EXISTS (
            ...
            ALTER TABLE parameter_versions ADD COLUMN strategy_name VARCHAR;
    ...
    BEGIN
        IF NOT EXISTS (
            ...
            ALTER TABLE trades ADD COLUMN strategy_name VARCHAR;
    ...
    BEGIN
        IF NOT EXISTS (
            ...
            ALTER TABLE adaptation_logs ADD COLUMN strategy_name VARCHAR;
...
def run_startup_migrations(db: Session) -> None:
    """Execute all idempotent DDL statements. Safe to call on every startup."""
    for stmt in _ADD_COLUMN_STMTS:
